...
public class Main {
    public static void main(String[] args) {
        ...
        // 카테고리가 여행인 책 제목 조회
        System.out.println("Q1. 카테고리가 여행인 책 제목 조회");
        bookList.stream()
                .filter( book -> book.getCategory().equals("여행"))
//                .forEach(System.out::println); //도서의 참조변수 저장값만 출력됨
                .forEach( book -> System.out.println("카테고리가 여행인 책 제목: " + book.getBookName()));

        // 가격이 16200원 이하인 책 제목 조회
        System.out.println("Q2. 가격이 16200원 이하인 책 제목 조회");
        bookList.stream()
                .filter( book -> book.getPrice() <= 16200)
                .forEach( book -> System.out.println("가격이 16200원 이하인 책 제목 조회 :" + book.getBookName()));

        // 책 제목에 "경제" 라는 용어가 들어간 책 제목 조회
        System.out.println("Q3. 책 제목에 \"경제\" 라는 용어가 들어간 책 제목 조회");
        bookList.stream()
                .filter( book -> book.getBookName().contains("경제"))
                .forEach( book -> System.out.println("책 제목에 \"경제\" 라는 용어가 들어간 책 제목 조회 :" + book.getBookName()));

        // 가격이 가장 비싼 책 가격 조회
        System.out.println("Q4. 책 목록 중 가장 비싼 금액");
        double maxPrice = bookList.stream()
                        .mapToDouble(Book::getPrice)
                .max().getAsDouble(); //getAS"자료형()"을 입력해야 출력 됨.
                System.out.println("책 목록 중 가장 비싼 금액" + maxPrice);

        System.out.println("Q5. 카테고리가 IT인 책들의 가격 합 조회");
        double sum = bookList.stream()
                .filter( book -> book.getCategory().equals("IT")) //필터로 IT카테고리 책 추출
                        .mapToDouble(Book::getPrice) //거른자료에서 위에 사용한 maptoDuble로 가격을 가져옴(price가 Double형)
                                .sum(); //가져온 값을 합친다
                System.out.println("카테고리가 IT인 책들의 가격 합 :" + sum);

        System.out.println("Q6. 카테고리가 IT 인 책들의 가격을 40% 할인하여 새로운 책 리스트 만들기");
        // IT 책 할인 이벤트!
        List<Book> discountedBookList = bookList.stream()
                .filter( book -> book.getCategory().equals("IT")) //필터로 할인 할 IT 카테고리 책 추출
                .map( book -> {book.setPrice(book.getPrice()*0.6); return book;}) //람다식 이용하여 책의 가격을 가져와서 수정 후 다시 리턴
                .toList(); //해당 리스트 저장(UnmodifiableList라 이후 데이터 수정은 불가능함)
